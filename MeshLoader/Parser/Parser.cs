...
namespace Visualization
{
    ...
    public class Parser
    {
        ...
        private uint cData;
        private ArrayList storerTable;
        private TokenType currentToken;
        private Mesh mesh;
        private Scanner scanner;
        private Stream fin;
        private FormattedStream fstream;
        ...
        private bool MatchAndAdvance(TokenType token)
        {
        ...
        private void MandatoryToken(TokenType token)
        {
            if (!MatchAndAdvance(token))
                throw new Exception("Parser: Bad input file.");
        }

        private bool NextTokenIsIJK()
        {
...
//			uint j_KEdgeStep, k_KEdgeStep;
            ...
            if (elementType == ElementType.IJKBrick)
            {
                ...
                for (i = 0; i < imax - 1; i++)
                {
                    ...
                    elementTable[currentElement].SetBrickRightFace(
                        (uint) (k * (jmax - 1) * imax + j * imax + i + 1 + jkFaceStart));
                    elementTable[currentElement].SetBrickBackFace((uint) (kjv + i));
                    elementTable[currentElement].SetBrickLeftFace(
                        (uint) (k * (jmax - 1) * imax + j * imax + i + jkFaceStart));
                    elementTable[currentElement].SetBrickTopFace(
                        (uint) (k * jmax * (imax - 1) + j * (imax - 1) + jstep + i + kiFaceStart));
                    elementTable[currentElement].SetBrickBottomFace(
                        (uint) (k * jmax * (imax - 1) + j * (imax - 1) + i + kiFaceStart));

                    elementTable[currentElement].VertInOrder = new uint[8];

                    elementTable[currentElement].VertInOrder[0] =
                        zone.Faces[elementTable[currentElement].GetBrickBottomFace()].Vertices[3];
                    elementTable[currentElement].VertInOrder[1] =
                        zone.Faces[elementTable[currentElement].GetBrickBottomFace()].Vertices[2];
                    elementTable[currentElement].VertInOrder[2] =
                        zone.Faces[elementTable[currentElement].GetBrickBottomFace()].Vertices[1];
                    elementTable[currentElement].VertInOrder[3] =
                        zone.Faces[elementTable[currentElement].GetBrickBottomFace()].Vertices[0];

                    elementTable[currentElement].VertInOrder[4] =
                        zone.Faces[elementTable[currentElement].GetBrickTopFace()].Vertices[3];
                    elementTable[currentElement].VertInOrder[5] =
                        zone.Faces[elementTable[currentElement].GetBrickTopFace()].Vertices[2];
                    elementTable[currentElement].VertInOrder[6] =
                        zone.Faces[elementTable[currentElement].GetBrickTopFace()].Vertices[1];
                    elementTable[currentElement].VertInOrder[7] =
                        zone.Faces[elementTable[currentElement].GetBrickTopFace()].Vertices[0];
        ...
        private static uint FaceTypeOf(TokenType feET)
        {
            switch (feET)
            {
                case TokenType.TRIANGLE:
        ...
        private static uint CalculateFaceCount(uint cElements, TokenType elementType)
        {
            switch (elementType)
            {
                case TokenType.TRIANGLE:
        ...
        private static uint CalculateEdgeCount(uint cElements, TokenType elementType)
        {
            switch (elementType)
            {
                case TokenType.TRIANGLE:
        ...
        private static ElementType GetElementType(TokenType elementType)
        {
            switch (elementType)
            {
                case TokenType.TRIANGLE:
